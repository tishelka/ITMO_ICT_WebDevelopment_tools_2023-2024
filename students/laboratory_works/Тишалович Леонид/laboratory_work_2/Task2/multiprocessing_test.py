...
def main():
    ...
    Создает процессы для парсинга новостей и каталога товаров и выполняет их параллельно.
    """
    print("Начало выполнения Multiprocessing парсинга")
    processes = [
        multiprocessing.Process(target=parse_news, args=('https://parsemachine.com/sandbox/news/',)),
        multiprocessing.Process(target=parse_catalog, args=('https://parsemachine.com/sandbox/catalog/',))
    ]
    ...
    for process in processes:
        process.join()
    print("Конец выполнения Multiprocessing парсинга")
